...
public class TimePicker extends FrameLayout {
	...
	private int mInterval = 1; // How many minutes to jump when using buttons.
	...
	public TimePicker(Context context, AttributeSet attrs, int defStyle) {
		...
		mMinutePicker.setOnChangeListener(new NumberPicker.OnChangedListener() {
			public void onChanged(NumberPicker spinner, int oldVal, int newVal) {
				...
				if (newVal > oldVal && !(oldVal == 0 && newVal == 59)) {
					Log.d(TAG, "Increasing oldVal by " + mInterval);
					mCurrentMinute = oldVal + mInterval;
					if (mCurrentMinute > 59)
						mCurrentMinute = 0;
				} else {
					Log.d(TAG, "Decreasing oldVal by " + mInterval);
					mCurrentMinute = oldVal - mInterval;
					if (mCurrentMinute < 0)
						mCurrentMinute = 60 - mInterval;
				}
				Log.d(TAG, "Current minutes: " + mCurrentMinute);

				int modulo = mCurrentMinute % mInterval;

				// Utilize the modulo value to adjust the minutes.
				if (modulo < (mInterval / 2.0)) {
					mCurrentMinute = mCurrentMinute - modulo;
				} else {
					mCurrentMinute = mCurrentMinute + mInterval - modulo;
				}
				Log.d(TAG, "Minutes aligned to interval: " + mCurrentMinute);
				onTimeChanged();
				updateMinuteDisplay();
			}
		});
	...
	public void setInterval(int interval) {
		Log.d(TAG, "Setting interval to: " + interval);
		mInterval = interval;
	}
